...
/* ═══════════════════════════════════════════
   ...
   Hero-centric, gamified between-session experience
   ═══════════════════════════════════════════ */
...
const PLAYER = {
  ...
  gearScore: 142,
  pikId: "PIK#7X2M9K",
  resources: { emberstone: 34, wyldroot: 12, seaglass: 8, dustiron: 21 },
};
...
const CODEX_ENTRIES = [
  { id: 1, title: "The Great Tree", realm: "The Wylds", category: "Lore", unlocked: true, content: "The source of all life and magic on the continent. Overseen by the Dryad Druids, the Great Tree's roots stretch beneath every realm of Elysendar, binding them together in ways that few understand." },
  { id: 2, title: "Kingvale Keep", realm: "Kingvale", category: "Locations", unlocked: true, content: "The castle and its high walls protect the royal family and the market square. Sturdy, old-world architecture \u2014 deep golds, white stone, and vibrant banners. The heart of military and diplomatic power." },
  { id: 3, title: "The Necro Rot", realm: "The Wylds", category: "Threats", unlocked: true, content: "A creeping corruption spreading through the forest. Necrotic wildlife, poisonous terrain, and a hidden temple lie at its source. The Druids struggle to contain it." },
  { id: 4, title: "Lochmaw Harbor", realm: "Lochmaw", category: "Locations", unlocked: true, content: "Built from the reclaimed wood of wrecked ships and overturned hulls. Loud, colorful, crude \u2014 home to Corsairs, pirates, traders, and thespians. The Serpent Slayers guild keeps the sea lanes safe." },
  { id: 5, title: "An'Haretti Ruins", realm: "Origin Sands", category: "Lore", unlocked: false },
  { id: 6, title: "The Dragon Pact", realm: "Desolate Peaks", category: "Lore", unlocked: false },
  { id: 7, title: "Order of the Drowned", realm: "Lochmaw", category: "Factions", unlocked: false },
  { id: 8, title: "The Rotten Bishop", realm: "Kingvale", category: "Characters", unlocked: false },
  { id: 9, title: "Bole of the Dryad", realm: "The Wylds", category: "Characters", unlocked: true, content: "The living spirit of the Great Tree made manifest. The Bole speaks in riddles, sees in seasons, and remembers every root and branch that has ever grown in Elysendar." },
  { id: 10, title: "Guardian Glaives", realm: "The Wylds", category: "Factions", unlocked: true, content: "Elite protectors of the Great Tree. They patrol the deep woods and answer only to the Tribunal. Few outsiders earn their trust." },
];

const CRAFT_RECIPES = [
  { id: 1, name: "Minor Elixir", desc: "Restores a small amount during sessions", time: 7200, cost: { wyldroot: 3 }, result: "consumable", icon: "\uD83E\uDDEA" },
  { id: 2, name: "Ember Ward", desc: "Temporary defense boost at LBE sessions", time: 14400, cost: { emberstone: 5, dustiron: 2 }, result: "gear_mod", icon: "\uD83D\uDD25" },
  { id: 3, name: "Wayfinder Charm", desc: "+10% XP for your next venue session", time: 28800, cost: { seaglass: 4, wyldroot: 2 }, result: "boost", icon: "\uD83E\uDDED" },
  { id: 4, name: "Sealed Crate Key", desc: "Opens sealed loot from LBE sessions", time: 3600, cost: { dustiron: 3 }, result: "key", icon: "\uD83D\uDD11" },
];
...
const DAILY = { name: "Study the Codex", desc: "Read 2 lore entries to prepare for the realms.", progress: 1, total: 2, xp: 50, claimed: false };
  ...
  return (
    <div style={{ padding: "0 20px 120px" }}>
      ...
      <Crd style={{ marginBottom: 16, background: "linear-gradient(135deg, rgba(99,102,241,0.08), rgba(139,92,246,0.04))", border: "1px solid rgba(99,102,241,0.15)" }}>
        ...
        <div style={{ marginTop: 8 }}><PBar value={DAILY.progress} max={DAILY.total} color="#a78bfa" height={4} /></div>
      </Crd>

      {/* Hero Card */}
      <div style={{ padding: 28, borderRadius: 16, background: `linear-gradient(180deg, ${PLAYER.tierColor}08 0%, rgba(0,0,0,0.4) 100%)`, border: `1px solid ${PLAYER.tierColor}25`, marginBottom: 20, textAlign: "center", position: "relative" }}>
        ...
        <>
          ...
          {SEALED_LOOT.map(l => (
            <Crd key={l.id} style={{ marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button style={{ padding: "6px 12px", borderRadius: 8, background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", color: "#a78bfa", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: FONT_B }}>Open</button>
            </Crd>
          ))}
      ...
      {/* Active Quests */}
      <SecTitle right={<span style={{ fontSize: 11, color: MUTED, fontFamily: FONT_B }}>{ACTIVE_QUESTS.length} active</span>}>Active Quests</SecTitle>
      {ACTIVE_QUESTS.slice(0, 2).map(q => (
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {Object.entries(PLAYER.resources).map(([key, ct]) => (
...
function QuestBoard() {
  ...
  const quests = filter === "active" ? ACTIVE_QUESTS : AVAILABLE_QUESTS;
  ...
  return (
    <div style={{ padding: "0 20px 120px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[["active", `Active (${ACTIVE_QUESTS.length})`], ["available", `Available (${AVAILABLE_QUESTS.length})`]].map(([id, label]) => (
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {!locked && filter === "available" && (
                  <button style={{ marginTop: 8, padding: "6px 16px", borderRadius: 8, background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", color: "#a78bfa", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: FONT_B }}>Accept Quest</button>
                )}
              </div>
            </div>
          </Crd>
...
function LoreCodex() {
  ...
  const filtered = catFilter === "All" ? CODEX_ENTRIES : CODEX_ENTRIES.filter(e => e.category === catFilter);
  const unlocked = CODEX_ENTRIES.filter(e => e.unlocked).length;
  ...
  return (
    <div style={{ padding: "0 20px 120px" }}>
      ...
      <p style={{ fontSize: 11, color: DIM, fontFamily: FONT_B, margin: "0 0 12px" }}>{unlocked}/{CODEX_ENTRIES.length} entries discovered</p>
      <PBar value={unlocked} max={CODEX_ENTRIES.length} color="#22c55e" height={4} />
      ...
      {!selected && filtered.map(entry => (
        <Crd key={entry.id} onClick={() => entry.unlocked ? setSelected(entry) : null} style={{ marginBottom: 8, cursor: entry.unlocked ? "pointer" : "default", opacity: entry.unlocked ? 1 : 0.4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: entry.unlocked ? "#fff" : MUTED, fontFamily: FONT_B }}>{entry.unlocked ? entry.title : "???"}</div>
...
function Workshop() {
  ...
  const startCraft = (r) => setTimers(p => ({ ...p, [r.id]: { started: Date.now(), duration: r.time } }));
  const fmtTime = (s) => { const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), sec = s % 60; return h > 0 ? `${h}h ${m}m` : m > 0 ? `${m}m ${sec}s` : `${sec}s`; };
  const canAfford = (cost) => Object.entries(cost).every(([k, v]) => (PLAYER.resources[k] || 0) >= v);
            ...
            return (
              <Crd key={id} style={{ marginBottom: 10, background: done ? "rgba(34,197,94,0.06)" : SURFACE, border: `1px solid ${done ? "rgba(34,197,94,0.2)" : BORDER}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  {done && <button style={{ padding: "8px 14px", borderRadius: 8, background: "rgba(34,197,94,0.2)", border: "1px solid rgba(34,197,94,0.3)", color: "#22c55e", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT_B }}>Collect</button>}
                </div>
              </Crd>
            );
          })}
